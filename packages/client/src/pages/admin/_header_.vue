...
<div ref="el" class="fdidabkc" :style="{ background: bg }" @click="onClick">
	<template v-if="metadata">
		...
		<div class="tabs">
			<button v-for="tab in tabs" :key="tab.key ?? tab.title" :ref="(el) => tabRefs[tab.key] = el" v-tooltip.no-delay="tab.title" class="tab _button" :class="{ active: tab.key != null && tab.key === props.tab }" @mousedown="(ev) => onTabMousedown(tab, ev)" @click="(ev) => onTabClick(tab, ev)">
	...
	<div class="buttons right">
		<template v-if="actions">
			<template v-for="action in actions" :key="action.text">
				...
				<button v-else v-tooltip.no-delay="action.text" class="_button button" :class="{ highlighted: action.highlighted }" @click.stop="action.handler" @touchstart="preventDrag"><i :class="action.icon"></i></button>
			</template>
		</template>
	</div>
</div>
