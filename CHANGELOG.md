...
- MFMのサジェストをより実用に即したものに
- 日本语 (怪)を追加

## akatsukey v2.3
- グループ機能の復活
- Admin,Moderatorの表示を変更
- Moderatorからいくつか権限を剥奪
- モバイルの下のバーを変更
- 下のバーにreloadボタンを表示し、そこが点灯するため不要になったstream-indicatorを削除
- ブラーがデフォルトでオフ
- パクる機能
- リアクションサイズを大きく
- 絵文字ピッカーを投稿画面の下に生やす(optout式)
- 投稿画面で画像をクロップする機能
- 権限があれば絵文字右クリックでインポートすることができる機能
- ソーシャルにグローバルを含むようにし、ソーシャル+に改名
- 管理者とモデレーターが他人のRenoteを解除できるように
- インスタンス情報を左側に移動
- 公開範囲別にRenoteボタンを追加
...
- v13MFMに対応(KaTeX廃止なし)
- Tabler Iconsに置換
- フォロー、フォロワーのページでフォローされていることを表示
- 入力画面でEnterキーを押したときにそのまま入力されてしまう不具合の修正
- 脆弱性の修正
- 枚数が多いときに異常に画像が小さくなる不具合の修正
- 5つ以上の場合は折り畳むように
- 自鯖のURLが表示されない不具合の修正
- リアクション一覧詳細ダイアログ
- 引用内の文章をnyaizeしないように
