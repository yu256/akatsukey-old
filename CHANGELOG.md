...
-->
## akatsukey-v1.4.5
- 汚いので画像クロップ機能を以前のものに戻した
- 部分リロード機能を廃止（ユースケースがないため）
- AiScriptコンソールウィジェットの高さを指定できるように
- ノートをダブルクリックして削除できるように(opt-out)
- VRChatAPI
	- ask meのフレンドを表示しないオプションを追加
	- friendsをソートして返すように
	- https://github.com/yu256/vrcapi_proxy に対応・移行
	- ユーザー検索機能
	- フレンド申請機能
	- 設定ページを分離
	- ワールド情報ページ
...
- VRChatAPI
	...
	- 不要なprops変数を削除
	- エラーハンドリングを強化
	- ユーザーとインスタンスのオーナーが違う場合のみオーナーのユーザー情報をfetchするように
- 不要なrefを削除
- 投稿フォームのファイルメニューからドライブのファイルを削除できるように
- 全角数字で終わるノートの数字引用に対応

## akatsukey-v1.4
- VRChatAPIを使用した機能を追加
	- オンラインのフレンドを表示するウィジェットを追加
	- ウィジェットのアイコンクリック時に詳細を表示するように
...
- モバイルのボトムバーのリロードの挙動を変更できるように
- モバイルではストリーミングインジケーターの代わりにリロードボタンを点灯させるように
- reactedのdrop-shadowを廃止（描画がおかしいため）
- MkEmojiPickerの下部に余白がある問題の修正
- メンションが上に偏っている問題の修正
...
- quicktime（mov）も再生できるように
- パクる・数字引用機能(Opt-in)
- フォローしているユーザーなら鍵ノートでもアンテナにひっかかるように
- メンションがpadding分はみ出る問題の修正
- デッキUIにリロードボタンを追加（必要なコンポーネントのみリロードします）
- DLsiteのRJ番号をリンク化するMFM for v13

## akatsukey-v1
- 負荷軽減とユーザビリティ向上のため少なくとも自鯖には必要のないと判断した機能をいろいろrevert
	- 実績
	- reaction acceptance
	- sensitive word
	- アンテナの自動停止
